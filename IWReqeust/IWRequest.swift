...
class IWRequest: NSObject {
    
    // 开启调试模式, 开启后输出所有信息
    static var openDebugMode: Bool = false
    // 开启提示模式, 开启后输出关键信息
    static var openPromptMode: Bool = true
    
    typealias IWRequestSuccessedHandler = (Data?) -> Void
    typealias IWRequestFailedHandler = (Error?) -> Void
...
extension IWRequest {
    ...
    // MARK: Class POST
    class func post(_ url: String, parameters: Any? = nil, desc: String? = nil) -> IWRequestResult {
        let request = IWRequest.request()
        request.config(post: url.toURL, parameters: parameters, desc: desc)
        return request.requestResult()
    }
    // MARK: Class GET
    class func get(_ url: String, parameters: Any? = nil, desc: String? = nil) -> IWRequestResult {
        let request = IWRequest.request()
