...
extension ViewControllerDelegate: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        switch _viewModel.subItemModel(with: indexPath).acid {
        case "naver":       iw.naver.url("./NaverViewController?title=URL跳转解决方案")
        case "permission":  iw.naver.url("./PermissionsViewController")
        ...
        case "login": do {
            currentViewController?.iwe.modal(IWLoginNavController())
        }
        default: break
        }
    }
    ...
    private func calcHeightForRowWithiOS8(_ indexPath: IndexPath) -> CGFloat {
        if let updateDesc = _viewModel.subItemModel(with: indexPath).sub as NSString? {
