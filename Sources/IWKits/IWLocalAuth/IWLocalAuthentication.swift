...
public class IWLocalAuthentication: NSObject {
    ...
    private var runningLaContext: LAContext?
    
    /// (旧的认证信息, 通过 initLocalAuthentication 进行初始化, 之后每次指纹认证成功都会刷新一遍).
    private var oldDomainState: Data?
    ...
    ///   - handler: 认证结果
    public final func authentication(tips: String? = nil, localizedReason: String = "安全校验", _ handler: @escaping ((_ authResult: AuthenticationResult) -> Void)) -> Void {
